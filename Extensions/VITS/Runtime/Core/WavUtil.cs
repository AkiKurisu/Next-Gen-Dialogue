...
namespace NextGenDialogue.VITS
{
    ...
    public static class WavUtil
    {
        ...
        private static void WriteHeader(byte[] stream, AudioClip clip, uint length, uint samples)
        {
            ...
            var byteRate = BitConverter.GetBytes(hz * channels * two); // sampleRate * bytesPerSample*number of channels, here 44100*2*2
            AddDataToBuffer(stream, ref offset, byteRate);

            var blockAlign = (ushort)(channels * two);
            ...
            var subChunk2 = BitConverter.GetBytes(samples * two);
