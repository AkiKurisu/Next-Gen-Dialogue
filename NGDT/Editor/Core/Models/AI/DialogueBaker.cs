...
namespace Kurisu.NGDT.Editor
{
    public class DialogueBaker
    {
        ...
        public static async UniTask AutoGenerateDialogue(DialogueGraphView graphView)
        {
            ...
            async Task<bool> Generate(IReadOnlyList<ContainerNode> inContainers, ContainerNode inBakeContainer, CancellationToken ct, int currentDepth, int maxDepth)
            {
                step++;
                if (!await baker.Bake(inContainers, inBakeContainer, ct)) return false;
                if (currentDepth >= maxDepth) return true;
                // Append current bake to last
                inContainers = new List<ContainerNode>(inContainers)
                {
                    inBakeContainer
                };
                int count = inBakeContainer is PieceContainer ? option : 1;
        ...
        public async Task<bool> Bake(IReadOnlyList<ContainerNode> containerNodes, ContainerNode bakeContainerNode, CancellationToken ct)
        {
            ...
            try
            {
                return await GenerateDialogue(bakeContainerNode, _builder, ct);
            }
        ...
        private static async UniTask<bool> GenerateDialogue(ContainerNode containerNode, AIPromptBuilder builder, CancellationToken ct)
        {
            try
            {
                var llm = CreateLLM();
        ...
        private static bool TrySetPrompt(ContainerNode containerNode, out string prompt)
        {
            ...
            if (containerNode.TryGetModuleNode<PromptPresetModule>(out ModuleNode promptPresetModule))
            {
            ...
            if (containerNode.TryGetModuleNode<CharacterPresetModule>(out ModuleNode presetModule))
            {
